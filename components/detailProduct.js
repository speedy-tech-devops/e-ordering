...
const DetailProduct = (props) => {
    ...
    const onChangeAddtoOrder = (e, id, limit, data, cho_id, choice_limit) => {
        ...
        let checkLimit = dataItem.attributes;
        let checkSelectOptions = [...options_detail, { ch_id: cho_id, ...data, min: choice_limit }].map(e => e.ch_id)
        const counts = {};
        checkSelectOptions.forEach(function (x) { counts[x] = (counts[x] || 0) + 1; });
        // const counts = {};
    ...
    useEffect(() => {
        ...
        const counts = {};
        checkSelectOptions.forEach(function (x) { counts[x] = (counts[x] || 0) + 1; });
        let limit = dataItem.attributes.filter((limit) => {
            if (counts[limit._id] == undefined) {
                counts[limit._id] = 0
            }
            return limit.choice_min > counts[limit._id]
            ...
            >
                ...
                <div className={style.scrollDtail}>
                    ...
                    <Accordion className='AccordionCustom' defaultActiveKey={[0, 1, 2]} alwaysOpen>
                        {
                            dataItem.attributes && dataItem.attributes.map((attribute, i) => {
                                return (
                                    <Accordion.Item eventKey={i}>
                                        <Accordion.Header>
                                            <p className={style.subMenu}> {attribute.choice_min > 0 && <span style={{ color: "red", fontSize: '20px' }}>* </span>}{attribute?.name[locale] ? attribute.name[locale] : attribute.name["th"]} <br /> {attribute.choice_min != 0 && <span style={{ display: "inline-block" }}>{t.EZ2001} {attribute.choice_min} {t.Item}</span>} {attribute.choice_limit >= 0 && <span style={{ display: "inline-block" }}>{t.EZ2002} {attribute.choice_limit} {t.Item}</span>}</p>
                                        </Accordion.Header>
                                        <Accordion.Body>
                                            {
                                                attribute.choice_limit <= 1 ?
                                                            ...
                                                            attribute.options.map((opt, i) => {
                                                                return (
                                                                    ...
                                                                    <div className="form-check mb-3">
                                                                        <input className="form-check-input" type="radio" name={attribute._id} id={"radio_" + i} value={opt._id} onClick={(e) => { onChangeAddtoOrder(e, "check_" + attribute._id, attribute.choice_limit, opt, attribute._id, attribute.choice_min) }} />
                                                    ...
                                                    <div className='optionItem'>
                                                        {
                                                            attribute.options.map((opt, i) => {
                                                                return (
                                                                    ...
                                                                    <div className="form-check mb-3">
                                                                        <input className={"form-check-input" + " check_" + attribute._id} type="checkbox" id={"check_" + opt._id} value={opt._id} onClick={(e) => { onChangeAddtoOrder(e, "check_" + attribute._id, attribute.choice_limit, opt, attribute._id, attribute.choice_min) }} />
